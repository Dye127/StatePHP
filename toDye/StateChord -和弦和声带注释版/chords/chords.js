...
Page({
  ...
  newGet: function () {
    ...
    // console.log(that.data.preDownPros)
    // console.log("new",that.data.tmpProblemsSrc)
   ...
   // console.log(res.data)
    ...
    //innerAudioContext.src = src + body.chord + '.mp3'
    innerAudioContext.src = that.data.tmpProblemsSrc[that.data.num - 1]
    // console.log("tmp file:" + that.data.tmpProblemsSrc[that.data.num - 1])
  ...
  lastGet: function () {
    ...
    this.setData({
      // answer0: app.globalData.chordType[current[2][0]],
      // answer1: app.globalData.chordType[current[2][1]],
      currentChordDetail: tmp
    })
    innerAudioContext.stop() //防止多次播放的影响
    //  innerAudioContext.src = src + this.data.proRecord[this.data.num][1] + '.mp3'
  ...
  checkAnswer: function (e) {
   ...
   // console.log(this.data.proRecord)
    this.setData({
      ans: rightAnswer,
      choose: cId
  ...
  setStatu: function (currStatuPos, oldStatuPos) {
    ...
    this.setData({
      ...
      flag: currStatu[4],
      //disabledChoose0: currStatu[4],
      //disabledChoose1: currStatu[5],
  ...
  preDownloadFile: function (srcArr) {
    ...
    var typeFile = '.mp3'
    for (var x in srcArr) {
      //console.log(srcArr[x])
      this.downloadFile(src + srcArr[x].data.chord + typeFile)
      console.log(srcArr[x].data.chord)
      // console.log("服务器mp3:",src + srcArr[x].data.chord + typeFile)//mine
    }
  ...
  getProblems: function (chordType) {
    var that = this
    //获取题列表
    //var problems = that.data.preDownPros
    // that.preDownloadFile(problems)
    wx.request({
      header: {
